// ============================================================================
// SakuraEDL - MainWindow.MediaTek.cs
// MediaTek platform WPF integration
// ============================================================================
...
namespace SakuraEDL
{
    public partial class MainWindow
    {
        ...
        private bool _mtkModuleInitialized;

        // MTK 日志级别: 0=关闭, 1=基本, 2=详细, 3=调试
        private int _mtkLogLevel = 2;
        private long _mtkLastProgressUiTick;
        private int _mtkLastProgressPercentage = -1;
        ...
        private void InitializeMediaTekModule()
        {
            ...
            try
            {
                ...
                // 加载漏洞类型列表
                LoadMtkExploitTypes();
                mtkSelectBootMode.SelectedIndexChanged += MtkSelectBootMode_SelectedIndexChanged;
                mtkChkAutoStorage.CheckedChanged += MtkChkAutoStorage_CheckedChanged;
                mtkInputDA.DoubleClick += MtkInputDA_DoubleClick;
                mtkInputScatter.DoubleClick += MtkInputScatter_DoubleClick;
                mtkInputAuth.DoubleClick += MtkInputAuth_DoubleClick;
                mtkSelectAuthMethod.SelectedIndexChanged += MtkSelectAuthMethod_SelectedIndexChanged;
        ...
        private void LoadMtkChipList()
        {
            ...
            foreach (var chip in chips)
            {
                ...
                mtkSelectChip.Items.Add(new CompatSelectItem(displayName) { Tag = chip.HwCode });
            }

            // 添加自动检测选项
            mtkSelectChip.Items.Insert(0, new CompatSelectItem("Auto Detect") { Tag = (ushort)0 });
        ...
        private void LoadMtkExploitTypes()
        {
            ...
            // 添加漏洞类型选项
            mtkSelectExploitType.Items.Add(new CompatSelectItem("Auto") { Tag = "Auto" });
            mtkSelectExploitType.Items.Add(new CompatSelectItem("Carbonara") { Tag = "Carbonara" });
            mtkSelectExploitType.Items.Add(new CompatSelectItem("AllInOne Signature") { Tag = "AllinoneSignature" });
            mtkSelectExploitType.Items.Add(new CompatSelectItem("None") { Tag = "None" });
        ...
        /// 芯片选择改变时更新漏洞类型
        /// </summary>
        private void MtkSelectChip_SelectedIndexChanged(object? sender, EventArgs e)
        {
        ...
        private void UpdateExploitTypeForSelectedChip()
        {
            ...
            var selectedItem = mtkSelectChip.SelectedValue as CompatSelectItem;
            ...
            {
                var item = mtkSelectExploitType.Items[i] as CompatSelectItem;
            ...
            {
                mtkBtnExploit.Text = "Run Exploit";
                mtkBtnExploit.Enabled = _mtkIsConnected;
            }
            else
            {
        ...
        private async void MtkBtnConnect_Click(object? sender, EventArgs e)
        {
            ...
            try
            {
                ...
                _mtkService.OnProgress += (current, total) => MtkUpdateProgress(current, total);
                ...
                if (success)
                {
                    ...
                    // 加载 DA (如果使用漏洞利用)
                    if (mtkChkExploit.IsChecked == true)
                    {
        ...
        private async void MtkBtnWritePartition_Click(object? sender, EventArgs e)
        {
            ...
            {
                folderDialog.Description = "Choose a folder that contains partition images";
                ...
                try
                {
                    ...
                    foreach (var partition in selectedPartitions)
                    {
                        ...
                        if (mtkChkSkipUserdata.IsChecked == true &&
                    ...
                    if (mtkChkRebootAfter.IsChecked == true)
                    {
                        MtkLogInfo("正在重启设备...");
        ...
        private async void MtkBtnReadPartition_Click(object? sender, EventArgs e)
        {
            ...
            using (var folderDialog = new System.Windows.Forms.FolderBrowserDialog())
            {
                folderDialog.Description = "Choose a save location";
                if (folderDialog.ShowDialog() != System.Windows.Forms.DialogResult.OK)
                    return;
        ...
        private async void MtkBtnExploit_Click(object? sender, EventArgs e)
        {
            ...
            // 获取选择的漏洞类型
            var selectedExploitItem = mtkSelectExploitType.SelectedValue as CompatSelectItem;
        ...
        private void MtkChkSelectAll_CheckedChanged(object? sender, EventArgs e)
        {
            foreach (ListViewItem item in mtkListPartitions.Items)
            {
                item.Checked = mtkChkSelectAll.IsChecked == true;
            }
        }
        ...
        private void MtkInputDaFile_SuffixClick(object? sender, EventArgs e)
        {
            using (var openDialog = new System.Windows.Forms.OpenFileDialog())
            {
                ...
                openDialog.Title = "Select DA files (DA1/DA2 can be selected together)";
        ...
        private void MtkUpdateProgress(int current, int total, string? statusText = null)
        {
            SafeInvoke(() =>
            {
                ...
                if (total > 0)
                {
                    ...
                    if (!MtkShouldRenderProgressUpdate(percentage))
                    {
                        return;
                    }
        ...
        private void MtkUpdateProgressWithBytes(long currentBytes, long totalBytes, string operation)
        {
            SafeInvoke(() =>
            {
                ...
                if (totalBytes > 0 && !MtkShouldRenderProgressUpdate(percentage))
                {
                    return;
                }
        ...
        private bool MtkShouldRenderProgressUpdate(int percentage)
        {
            long now = Environment.TickCount64;
            bool isTerminal = percentage <= 0 || percentage >= 100;

            if (!isTerminal &&
                percentage == _mtkLastProgressPercentage &&
                now - _mtkLastProgressUiTick < 50)
            {
                return false;
            }
            ...
            _mtkLastProgressUiTick = now;
            _mtkLastProgressPercentage = percentage;
            return true;
        }

        private void MtkResetProgressThrottle()
        {
            _mtkLastProgressUiTick = 0;
            _mtkLastProgressPercentage = -1;
        }
        ...
        private void MtkInputScatterFile_SuffixClick(object? sender, EventArgs e)
        {
            using (var openDialog = new System.Windows.Forms.OpenFileDialog())
            {
                ...
                openDialog.Title = "Select an MTK scatter file";
