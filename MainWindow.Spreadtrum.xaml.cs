...
namespace SakuraEDL
{
    public partial class MainWindow
    {
        ...
        private bool _spreadtrumModuleInitialized;
        private uint _selectedChipId = 0;
        private long _sprdLastProgressUiTick;
        private int _sprdLastProgressPercentage = -1;
        ...
        {
            if (_spreadtrumModuleInitialized)
            {
                return;
            }
            ...
            try
            {
                _spreadtrumModuleInitialized = true;
            ...
            {
                _spreadtrumModuleInitialized = false;
        ...
        private void InitializeChipSelector()
        {
            ...
            foreach (var item in items) sprdSelectChip.Items.Add(item);
            sprdSelectChip.SelectedIndex = 0;
        ...
        private void UpdateDeviceList(string chipName)
        {
            ...
            {
                Dispatcher.BeginInvoke(new Action(() => UpdateDeviceList(chipName)), System.Windows.Threading.DispatcherPriority.Background);
                return;
            }
            ...
            // 一次性更新
            sprdSelectDevice.Items.Clear();
            foreach (var item in items) sprdSelectDevice.Items.Add(item);
        ...
        private void BindSpreadtrumEvents()
        {
            ...
            input2.DoubleClick += (s, e) =>
            {
                ...
                {
                    ofd.Title = "Select an FDL1 file";
                    ofd.Filter = "FDL Files (*.bin)|*.bin|All Files (*.*)|*.*";
            ...
            sprdChkSelectAll.CheckedChanged += (s, e) =>
            {
                ...
                {
                    item.Checked = sprdChkSelectAll.IsChecked == true;
                }
            };
            ...
            _spreadtrumController.OnPacLoaded += pac =>
            {
                SafeInvoke(() =>
                {
                    ...
                    foreach (var file in pac.Files)
                    {
                        ...
                        // 如果勾选了跳过 Userdata
                        if (sprdChkSkipUserdata.IsChecked == true && file.Type == PacFileType.UserData)
                            shouldCheck = false;
            ...
            _spreadtrumController.OnProgress += (current, total) =>
            {
                SafeInvoke(() =>
                {
                    ...
                    bool isTerminal = percent <= 0 || percent >= 100;

                    if (!isTerminal &&
                        percent == _sprdLastProgressPercentage &&
                        now - _sprdLastProgressUiTick < 50)
                    {
                        return;
                    }
                    ...
                    _sprdLastProgressUiTick = now;
                    _sprdLastProgressPercentage = percent;
            ...
            using (var ofd = new System.Windows.Forms.OpenFileDialog())
            {
                ...
                if (ofd.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    if (result == System.Windows.Forms.DialogResult.Yes)
                    {
                        ...
                        if (success)
                        {
                            System.Windows.Forms.MessageBox.Show($"分区 {partitionName} 刷写成功！", "完成", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        }
                    }
                }
            }
        ...
        private async Task SprdFlashMultiplePartitionsAsync(List<string> partitionNames)
        {
            ...
            {
                fbd.Description = $"Choose a folder that contains image files.\nAutomatic match list: {string.Join(", ", partitionNames)}";
                ...
                if (fbd.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    var result = System.Windows.Forms.MessageBox.Show(msg, "确认刷写", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
        ...
        private void SprdBrowsePac()
        {
            using (var ofd = new System.Windows.Forms.OpenFileDialog())
            {
                    ofd.Title = "Select a Spreadtrum PAC firmware package";
                    ofd.Filter = "PAC Firmware Packages (*.pac)|*.pac|All Files (*.*)|*.*";
        ...
        private async System.Threading.Tasks.Task SprdWritePartitionAsync()
        {
            ...
            if (partitions.Count == 1)
            {
                ...
                using (var ofd = new System.Windows.Forms.OpenFileDialog())
                {
                    ofd.Title = $"Select a file to write to partition {partName}";
                    ofd.Filter = "镜像文件 (*.img;*.bin)|*.img;*.bin|所有文件 (*.*)|*.*";
            ...
            using (var fbd = new System.Windows.Forms.FolderBrowserDialog())
            {
                fbd.Description = $"Choose a folder that contains {partitions.Count} partition images.\n(File names must match partition names, for example boot.img)";
        ...
        private async System.Threading.Tasks.Task SprdWriteSingleImageAsync(List<string> selectedPartitions)
        {
            ...
            else
            {
                ...
                using (var ofd = new System.Windows.Forms.OpenFileDialog())
                {
                    ...
                    ofd.Filter = "Image Files (*.img;*.bin)|*.img;*.bin|All Files (*.*)|*.*";
        ...
        private async System.Threading.Tasks.Task SprdWriteMultipleImagesAsync()
        {
            using (var ofd = new System.Windows.Forms.OpenFileDialog())
            {
                ofd.Title = "Select image files (file names should match partition names)";
                ofd.Filter = "Image Files (*.img;*.bin)|*.img;*.bin|All Files (*.*)|*.*";
                ofd.Multiselect = true;
        ...
        private async System.Threading.Tasks.Task SprdWriteEntirePacAsync()
        {
            ...
            // 刷机后重启
            if (success && sprdChkRebootAfter.IsChecked == true)
            {
        ...
        private async System.Threading.Tasks.Task SprdExtractPacAsync()
        {
            ...
            using (var fbd = new System.Windows.Forms.FolderBrowserDialog())
            {
                fbd.Description = "Choose an extraction folder";
                if (fbd.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
        ...
        private void UpdateSprdInfoPanel()
        {
            ...
            else if (!string.IsNullOrEmpty(_detectedSprdPort))
            {
                // 检测到设备但未连接
                uiLabel9.Text = "Platform: Spreadtrum";
                uiLabel11.Text = "Chip: Waiting for initialization";
                uiLabel13.Text = "Stage: --";
                uiLabel10.Text = "IMEI: --";
                uiLabel3.Text = $"Port: {_detectedSprdPort}";
                uiLabel12.Text = $"Mode: {_detectedSprdMode}";
                uiLabel14.Text = "Status: Waiting";
            }
            else
            {
                // 未检测到设备
        ...
        private async Task SprdBackupCalibrationAsync()
        {
            ...
            using (var fbd = new System.Windows.Forms.FolderBrowserDialog())
            {
                fbd.Description = "Choose a calibration backup folder";
                if (fbd.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
        ...
        private async Task SprdRestoreCalibrationAsync()
        {
            ...
            using (var fbd = new System.Windows.Forms.FolderBrowserDialog())
            {
                fbd.Description = "Choose a folder that contains the calibration backup";
                if (fbd.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    var result = System.Windows.Forms.MessageBox.Show(
                        "确定要恢复校准数据吗？\n\n此操作将覆盖设备当前的校准数据！",
        ...
        private async Task SprdWriteNvAsync(ushort itemId, string itemName, int expectedLength)
        {
            ...
            if (success)
            {
                ...
                System.Windows.Forms.MessageBox.Show($"{itemName} 写入成功！", "成功", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
            {
                AppendLog($"[展讯] {itemName} 写入失败", Color.Red);
                System.Windows.Forms.MessageBox.Show($"{itemName} 写入失败！", "失败", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private async Task SprdReadCustomNvAsync()
        {
            ...
            if (!ushort.TryParse(input.Trim(), out itemId))
            {
                ...
                if (input.StartsWith("0x", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    {
                        System.Windows.Forms.MessageBox.Show("ID 格式错误！", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
                    }
                }
                else
            ...
            if (data != null && data.Length > 0)
            {
                ...
                System.Windows.Forms.MessageBox.Show(msg, "NV 读取", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
            {
                AppendLog($"[展讯] 读取 NV[{itemId}] 失败", Color.Red);
                System.Windows.Forms.MessageBox.Show($"读取 NV[{itemId}] 失败！\n\n该项可能不存在", "失败", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
        ...
        private async Task SprdWriteCustomNvAsync()
        {
            ...
            if (!ushort.TryParse(idInput.Trim(), out itemId))
            {
                if (idInput.StartsWith("0x", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    catch { System.Windows.Forms.MessageBox.Show("ID 格式错误！", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error); return; }
                }
                else
                {
                    System.Windows.Forms.MessageBox.Show("ID 格式错误！", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
            ...
            if (success)
            {
                ...
                System.Windows.Forms.MessageBox.Show($"NV[{itemId}] 写入成功！", "成功", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
            {
                AppendLog($"[展讯] NV[{itemId}] 写入失败", Color.Red);
                System.Windows.Forms.MessageBox.Show($"NV[{itemId}] 写入失败！", "失败", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private async Task SprdBackupSelectedPartitionsAsync()
        {
            ...
            using (var fbd = new System.Windows.Forms.FolderBrowserDialog())
            {
                fbd.Description = "Choose a folder to save the backup";
